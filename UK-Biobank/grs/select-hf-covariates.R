...
covs_hf_qc <- covs_hf[keep,]

missing_cov <- apply(covs_hf_qc, 1, function(x) any(is.na(x)))
covs_hf_complete <- covs_hf_qc[!missing_cov,]

# get IDs of related samples
keepRelatedness <- dplyr::filter(relatedness,
                                 ID1 %in% covs_hf_complete$eid | ID2 %in% covs_hf_complete$eid)
...
covs_hf_norelated <- covs_hf_complete[!covs_hf_complete$eid %in% relatedIDs$failIDs$IID,]
...
overview_id$missing_covariates <- covs_hf_qc$eid[missing_cov]
overview_id$related <- relatedIDs$failIDs$IID
...
overview$missing_covariates <- length(covs_hf_qc$eid[missing_cov])
